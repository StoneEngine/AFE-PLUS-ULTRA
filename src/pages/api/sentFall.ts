...
export default async function handle(req: NextApiRequest, res: NextApiResponse<Data>) {
    if (req.method === 'PUT' || req.method === 'POST') {
        try {
            ...
            const userId = Number(body.users_id);
            ...
            const ctxResult = await withUserContext(userId, async (tx) => {
                ...
                const takecareperson = await tx.takecareperson.findFirst({
                    where: { takecare_id: takecareId, takecare_status: 1 }
                });
                ...
                const lastFall = await tx.fall_records.findFirst({
                    where: {
            ...
            if (ctxResult.shouldNotify) {
                const { user, takecareperson, notifyMessage } = ctxResult;
