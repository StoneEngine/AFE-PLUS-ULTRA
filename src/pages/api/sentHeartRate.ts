    ...
    if (req.method === 'PUT' || req.method === 'POST') {
        try {
            ...
            const ctxResult = await withUserContext(userId, async (tx) => {
                ...
                const settingHR = await tx.heartrate_settings.findFirst({
                    where: {
                ...
                const lastHR = await tx.heartrate_records.findFirst({
                    where: {
                        users_id: user.users_id,
                        takecare_id: takecareperson.takecare_id
                    },
                    orderBy: { heartrate_id: 'desc' }
                });
                ...
                if (lastHR) {
                    ...
                    await tx.heartrate_records.update({
                        where: { heartrate_id: lastHR.heartrate_id },
                        data: updateData
                    });
                } else {
                    await tx.heartrate_records.create({
                        data: {
                            users_id: user.users_id,
                            takecare_id: takecareperson.takecare_id,
                            ...recordData
            ...
            const { user, takecareperson, shouldSendNotification } = ctxResult;
